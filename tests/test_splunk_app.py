...
class SplunkAppTests(unittest.TestCase):
    ...
    def test_framework_dependent_selectors_and_defaults_are_safe(self) -> None:
        ...
        self.assertIn("inputlookup requirements.csv", requirements_view)
        ...
        self.assertIn("form.requirement_token=$click.value$", requirements_view)
        ...
        self.assertIn('open_check as "Open check"', requirements_view)
        self.assertIn('<option name="drilldown">cell</option>', requirements_view)
        self.assertIn('<condition field="Open check">', requirements_view)
        self.assertIn('eval open_check=requirement_key', requirements_view)
        self.assertIn('<link>/app/compliance_control_plane/check?form.framework_token=$framework_token$&amp;form.requirement_token=$click.value$</link>', requirements_view)
        ...
        measurements_view = (APP / "default/data/ui/views/measurements.xml").read_text(encoding="utf-8")
        self.assertNotIn("NOT data-tooltip", measurements_view)
        self.assertIn('NOT title="_*"', measurements_view)
        ...
        self.assertIn('<earliest>-24h</earliest>', technical_view)
        self.assertIn('<latest>now</latest>', technical_view)
