...
class SplunkAppTests(unittest.TestCase):
    ...
    def test_framework_dependent_selectors_and_defaults_are_safe(self) -> None:
        ...
        self.assertNotIn('<unset token="requirement_token"', check_view)
        ...
        self.assertIn("form.requirement_token=$row.requirement_key$", requirements_view)
        self.assertIn("'value' != &quot;&quot;", requirements_view)
        self.assertGreaterEqual(requirements_view.count('<row depends="$framework_selected$">'), 2)
        self.assertIn('<drilldown target="blank">', requirements_view)
        self.assertIn("<![CDATA[", requirements_view)
        ...
        self.assertIn('/app/compliance_control_plane/check?form.framework_token=$row.framework_key$&form.requirement_token=$row.requirement_key$', requirements_view)
